...
Whether to remove the preview channel(s) once the PR is closed (not the same as when a PR is merged).
If this is set to false, the preview channel will be removed on its previously set expiry time.
